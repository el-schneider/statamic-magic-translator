...
const targetSites = computed(() =>
    sites.value.filter((s) => s.handle !== currentSite.value),
)
...
function tryInject(): void {
    ...
    if (succeeded) {
        badgeInjected.value = true
        // Once injection succeeds, disconnect the observer — re-injection is
        // handled by watching the `sites` computed (via parent re-mount).
        observer?.disconnect()
        observer = null
    }
}
...
onMounted(() => {
    ...
    if (!badgeInjected.value) {
        // Watch for DOM changes so we can retry once the Sites panel renders
        observer = new MutationObserver(() => {
            if (!badgeInjected.value) tryInject()
        })
...
function openDialog(): void {
    ...
    const dialog = Statamic.$components.append('content-translator-dialog', {
        props: {
            ...
            sourceSite: currentSite.value ?? sites.value[0]?.handle ?? '',
            sites: sites.value,
        },
    })
...
<template>
    <div class="content-translator-fieldtype">
        ...
        -->
        <div
            v-if="!badgeInjected && sites.length > 0"
        ...
        >
            ...
            >
                ...
                >⚠</span>
                <span
                    v-else-if="site.last_translated_at"
