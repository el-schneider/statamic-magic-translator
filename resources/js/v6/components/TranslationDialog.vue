...
const dialogTitle = computed(() =>
    isBulk.value
        ? `Translate ${String(allEntryIds.value.length)} entries`
        : 'Translate',
)
...
const selectedLocales = ref<string[]>(
    // Pre-select: check locales that don't exist yet; uncheck existing ones.
    props.sites
        .filter((s) => {
            if (s.handle === props.sourceSite) return false
            const full = s as SiteMeta
            return !full.exists
        })
        .map((s) => s.handle),
)
...
const hasFailed = computed(() =>
    Object.values(localeState).some((s) => s.status === 'failed'),
)
    ...
    stopPolling = pollJobs(allJobIds, (jobs) => {
        for (const job of jobs) {
            ...
            localeState[handle] = {
                ...state,
                status: isTerminal
                    ? (job.status as 'completed' | 'failed')
                    : (job.status as 'pending' | 'running'),
                error: job.error ?? null,
                completedCount: isTerminal ? state.completedCount + 1 : state.completedCount,
            }
        }
    })
}
...
async function retryLocale(handle: string): Promise<void> {
    ...
    stopPolling?.()
    const existingIds = Object.values(localeState)
        .flatMap((s) => s.jobIds)
        .filter((id) => {
            // Keep IDs that are still running
            return true
        })
    ...
    <div class="fixed inset-0 z-[200] flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-dark-550 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div class="p-6 space-y-5 max-h-[65vh] overflow-y-auto">
                ...
                <div class="space-y-0.5">
                    ...
                    >
                        ...
                        >
                            <span>{{ site.name }}</span>
                            <span
                                v-if="(site as SiteMeta).is_stale"
                                class="ml-2 text-xs text-amber-500"
                            >⚠ outdated</span>
                            <span
                                v-else-if="(site as SiteMeta).last_translated_at"
                ...
                >
                    <template
                        v-for="(state, handle) in localeState"
                        :key="handle"
                    >
                        <p
