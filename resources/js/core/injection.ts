...
function findLocaleRows(panel: Element, version: 'v5' | 'v6'): Element[] {
    if (version === 'v5') {
        ...
        return Array.from(panel.querySelectorAll('div.text-sm')).filter((el) =>
            el.querySelector('.little-dot'),
        )
    } else {
        // In v6, rows are <button> elements that contain .little-dot
        return Array.from(panel.querySelectorAll('button')).filter((el) =>
            el.querySelector('.little-dot'),
        )
    }
}
...
function createBadge(site: SiteMeta): HTMLElement {
    ...
    if (!site.exists) {
        ...
        span.setAttribute(
            'style',
            'font-size:11px;color:#f59e0b;margin-left:6px;font-weight:500;',
        )
        if (site.last_translated_at) {
            span.setAttribute('title', `Last translated: ${relativeTime(site.last_translated_at)}`)
        }
    } else if (site.last_translated_at) {
        span.textContent = relativeTime(site.last_translated_at)
        span.setAttribute('title', `Last translated: ${site.last_translated_at}`)
        span.setAttribute(
            'style',
        ...
        span.setAttribute(
            'style',
            'font-size:11px;color:#6b7280;margin-left:6px;font-style:italic;',
        )
    }
...
export function injectBadges(
    ...
    for (const row of rows) {
        ...
        const siteStatus = localeStatuses.find(
            (s) => s.name === siteName || s.handle === siteName,
        )
        ...
        if (version === 'v6') {
            ...
            const badgesContainer =
                row.querySelector('.flex.items-center.gap-1\\.5') ??
                row.querySelector('.flex.items-center:last-child')
            if (badgesContainer) {
