...
it('drops null values from translated data before saving', function () {
    ['entry' => $entry, 'action' => $action] = setUpTranslationTest();

    Event::listen(AfterEntryTranslation::class, function (AfterEntryTranslation $event) {
        $event->translatedData['title'] = null;
    });
    ...
    // A null must not be written, or the localization would shadow the origin
    // value with an empty one.
    expect($fr->data()->has('title'))->toBeFalse();
});
